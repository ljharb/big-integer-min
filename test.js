...
test('requires a string', function (t) {
	t.throws(function () { bigIntMin('', 1); }, TypeError, 'throws on first non-string arg');
	t.throws(function () { bigIntMin(1, ''); }, TypeError, 'throws on second non-string arg');
	t.end();
});

test('requires a valid integer', function (t) {
	t.throws(function () { bigIntMin('', '123a'); }, TypeError, 'requires only digits');
	t.throws(function () { bigIntMin('', '123.3'); }, TypeError, 'requires only digits');
	t.end();
});

test('does not allow leading zeroes', function (t) {
	t.throws(function () { bigIntMin('123', '001'); }, TypeError, 'does not allow leading zeroes');
	t.end();
});
